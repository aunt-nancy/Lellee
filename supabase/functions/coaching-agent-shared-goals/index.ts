...
Deno.serve(async (request) => {
  ...
  return withAgentGuard(request, {
    agentKey: "coaching",
    capability: "coaching.shared_goals",
    ...
    handler: async ({ userId, userClient }) => {
      ...
      if (error) {
        console.error("Shared coaching goals lookup failed", error);
        return jsonResponse({ error: "Shared coaching goals unavailable" }, 500);
      }

      return jsonResponse({
        ok: true,
        agent: "coaching",
        mode: "limited_user_shared",
        goals: data || []
      });
    }
  });
