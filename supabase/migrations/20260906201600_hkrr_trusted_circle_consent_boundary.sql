-- Canonical Lellee runtime-project reconciliation: Trusted Circle consent boundary.
-- Target project is hkrrxscyhtxmbvxevfkw. No UI, billing, journal, pricing, or Auth changes.
...
begin
  ...
  if exists(select 1 from public.trusted_circle_relationships)
    ...
    raise exception 'Consent tables are no longer empty; inspect existing records before changing consent rules';
  end if;
...
create unique index if not exists circle_one_account_share
  on public.trusted_circle_shares(relationship_id,scope_key) where program_id is null;

alter table public.trusted_circle_relationships drop constraint if exists trusted_circle_relationships_program_id_fkey;
...
alter table public.trusted_circle_shares drop constraint if exists trusted_circle_shares_program_id_fkey;
alter table public.trusted_circle_shares add constraint trusted_circle_shares_program_id_fkey
  foreign key(program_id) references public.programs(id) on delete restrict;
alter table public.trusted_circle_shares drop constraint if exists circle_valid_share_window;
...
begin
  ...
  if p_email is null or length(btrim(p_email))=0 or length(p_email)>320 or p_role_key is null
  ...
  if rel.id is null then
    ...
    update public.trusted_circle_relationships set status='invited',invited_by=caller,invited_at=ts,
      accepted_at=null,revoked_at=null where id=rel.id;
  end if;
  return rel.id;
end $fn$;
...
begin
  ...
  if not found or NEW.owner_user_id is distinct from rel.owner_user_id or rel.status<>'active'
     or rel.revoked_at is not null or rel.accepted_at is null or rel.invited_by is distinct from rel.owner_user_id
...
begin
  for p in select policyname from pg_policies where schemaname='public'
    and tablename='trusted_circle_relationships' and cmd<>'SELECT'
  loop execute format('drop policy %I on public.trusted_circle_relationships',p.policyname); end loop;
  for p in select policyname from pg_policies where schemaname='public' and tablename='trusted_circle_shares'
  loop execute format('drop policy %I on public.trusted_circle_shares',p.policyname); end loop;
end $drop_write_policies$;

drop policy if exists circle_relationships_parties on public.trusted_circle_relationships;
create policy circle_relationships_parties on public.trusted_circle_relationships for select to authenticated
...
revoke all on table public.trusted_circle_relationships,public.trusted_circle_shares,
  public.trusted_circle_roles,public.trusted_circle_share_scopes from public,anon,authenticated;
...
revoke all on function public.invite_trusted_circle_member(text,text),
  public.respond_trusted_circle_relationship(uuid,text),public.accept_trusted_circle_invitation(uuid),
  public.revoke_trusted_circle_relationship(uuid),public.revoke_trusted_circle_share(uuid),
  public.guard_trusted_circle_share_insert() from public,anon,authenticated;
grant execute on function public.invite_trusted_circle_member(text,text),
  public.respond_trusted_circle_relationship(uuid,text),public.accept_trusted_circle_invitation(uuid),
  public.revoke_trusted_circle_relationship(uuid),public.revoke_trusted_circle_share(uuid) to authenticated,service_role;
...
begin
  foreach sig in array array[
    ...
    if has_function_privilege('anon',sig,'EXECUTE') then raise exception 'Anonymous lifecycle grant remains: %',sig; end if;
    if not has_function_privilege('authenticated',sig,'EXECUTE') then raise exception 'Authenticated lifecycle grant missing: %',sig; end if;
  end loop;
  if has_any_column_privilege('authenticated','public.trusted_circle_relationships','INSERT,UPDATE')
     or has_table_privilege('authenticated','public.trusted_circle_relationships','DELETE') then
  ...
  if not exists(select 1 from pg_trigger where tgrelid='public.trusted_circle_shares'::regclass
    and tgname='circle_validate_new_share' and not tgisinternal) then raise exception 'Share guard trigger missing'; end if;
end $verify$;
