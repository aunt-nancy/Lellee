...
let step=1,state={journey:null,journeys:[],path:'exploring',belief:'exploring'};
const JOURNEY_LABELS={recovery:'Recovery',caregiving:'Caregiving',reentry:'Returning Home','housing-stability':'Housing Stability','independent-living':'Building Independence','not-sure':'Lellee'};
const JOURNEY_REASONS={recovery:'Recovery offers support for substance-use change, recovery routines, coping tools and progress.',caregiving:'Caregiving helps with priorities, boundaries, organization, asking for help and sustainable routines.','reentry':'Returning Home supports practical next steps while rebuilding life after incarceration.','housing-stability':'Housing Stability helps organize housing needs, documents, appointments and manageable next steps.','independent-living':'Building Independence supports daily-living skills, routines, goals and greater self-direction.','not-sure':'Explore Lellee without choosing a journey. You can select a starting point whenever you are ready.'};

function selectedJourneys(){return state.journeys?.length?state.journeys:(state.journey?[state.journey]:[])}
function showStep(n){step=Math.max(1,Math.min(5,n));qa('.onboarding-step').forEach(x=>x.classList.toggle('active',Number(x.dataset.onboardingStep)===step));q('#onboardingBack')?.classList.toggle('hidden',step===1);q('#onboardingNext').textContent=step===5?'Finish Setup':'Continue';q('#onboardingStepLabel').textContent=step===1?(selectedJourneys().includes('recovery')?'STEP 1 OF 5':'STARTING POINT'):`STEP ${step} OF 5`;q('#onboardingProgressBar').style.width=step===1&&!state.journey?'10%':`${step*20}%`}
function syncJourneyChoice(){const chosen=selectedJourneys(),primary=chosen[0]||null,secondary=chosen.slice(1);state.journey=primary;qa('[data-onboarding-journey]').forEach(b=>b.classList.toggle('selected',chosen.includes(b.dataset.onboardingJourney)));const box=q('#onboardingRecommendation'),title=q('#onboardingRecommendationTitle'),copy=q('#onboardingRecommendationText'),extra=q('#onboardingSecondaryRoutes'),next=q('#onboardingNext');if(box)box.classList.toggle('hidden',!primary);if(title)title.textContent=primary==='not-sure'?'Explore without choosing':JOURNEY_LABELS[primary]||'';if(copy)copy.textContent=JOURNEY_REASONS[primary]||'';if(extra){extra.classList.toggle('hidden',!secondary.length);extra.textContent=secondary.length?`Also suggested: ${secondary.map(x=>JOURNEY_LABELS[x]).join(', ')}.`:''}if(next&&step===1)next.textContent=primary==='not-sure'?'Continue without choosing':chosen.includes('recovery')?'Review Recovery setup':chosen.length>1?'Use suggested journeys':primary?`Use ${JOURNEY_LABELS[primary]}`:'Continue';q('#onboardingStepLabel').textContent=chosen.includes('recovery')?'STEP 1 OF 5':'STARTING POINT';q('#onboardingProgressBar').style.width=primary?'20%':'10%'}
...
async function preload(){
 ...
 state.path=u.data?.recovery_approach||o.data?.recovery_path||'exploring';state.belief=u.data?.belief_preference||o.data?.belief_preference||'exploring';state.journey=['recovery','caregiving','reentry','housing-stability','independent-living','not-sure'].includes(o.data?.primary_intent)?o.data.primary_intent:null;state.journeys=state.journey?[state.journey]:[];
 ...
 qa('[data-onboarding-path]').forEach(b=>b.classList.toggle('selected',b.dataset.onboardingPath===state.path));qa('[data-onboarding-belief]').forEach(b=>b.classList.toggle('selected',b.dataset.onboardingBelief===state.belief));syncJourneyChoice();faithFields();
}
...
async function saveSetup(){
 ...
 for(const journey of selectedJourneys().filter(x=>x!=='not-sure')){const {data:started,error:startError}=await sb.rpc('start_selected_support_path',{p_topic_key:journey});if(startError||started?.status!=='started')return toast(startError?.message||`We could not start ${JOURNEY_LABELS[journey]}. Please try again.`,true)}
 const rs=await Promise.all([
  ...
  sb.from('onboarding_state').upsert({user_id:currentUser.id,completed:true,completed_at:new Date().toISOString(),primary_intent:state.journey||'recovery',recovery_path:state.path,belief_preference:state.belief},{onConflict:'user_id'})
 ]);
 ...
 data.name=name;data.recoveryDate=recoveryDate;data.goal=goal||'';data.city=city||'';data.state=st||'';data.faith=faith||'';data.denomination=den||'';if(typeof save==='function')save();if(typeof renderBasics==='function')renderBasics();q('#finalOnboardingOverlay')?.classList.add('hidden');if(state.journey&&state.journey!=='recovery'){try{sessionStorage.setItem('lellee:active-consumer-journey:v1',state.journey)}catch(_){}location.hash='wave1-beta-workspace';location.reload();return}showPage('today');toast('Your Lellee setup is ready.');
}
async function startSelectedJourney(){
 const name=q('#onboardingName').value.trim(),selected=selectedJourneys();if(!name)return toast('Add the name you want Lellee to use.',true);if(!selected.length)return toast('Choose what you would like support with.',true);
 const primary=selected[0],rs=await Promise.all([sb.from('profiles').update({display_name:name}).eq('id',currentUser.id),sb.from('onboarding_state').upsert({user_id:currentUser.id,completed:true,completed_at:new Date().toISOString(),primary_intent:primary},{onConflict:'user_id'})]);const err=rs.find(x=>x.error)?.error;if(err)return toast(err.message,true);
 if(primary==='not-sure'){q('#finalOnboardingOverlay')?.classList.add('hidden');showPage('program-switcher');toast('You can choose a journey whenever you are ready.');return}
 for(const journey of selected){const {data:started,error}=await sb.rpc('start_selected_support_path',{p_topic_key:journey});if(error||started?.status!=='started')return toast(error?.message||`We could not start ${JOURNEY_LABELS[journey]}. Please try again.`,true)}
 data.name=name;if(typeof save==='function')save();q('#finalOnboardingOverlay')?.classList.add('hidden');try{sessionStorage.setItem('lellee:active-consumer-journey:v1',primary)}catch(_){}location.hash='wave1-beta-workspace';location.reload();
}
function next(){if(step===1){if(!q('#onboardingName').value.trim())return toast('Add the name you want Lellee to use.',true);if(!state.journey)return toast('Choose what you would like support with.',true);if(!selectedJourneys().includes('recovery'))return startSelectedJourney();showStep(2);return}if(step===2&&!q('#onboardingRecoveryDate').value)return toast('Choose a recovery date or use today.',true);if(step<5)showStep(step+1);else saveSetup()}
