...
let step=1,state={journey:null,path:'exploring',belief:'exploring'};
...
function syncJourneyChoice(){qa('[data-onboarding-journey]').forEach(b=>b.classList.toggle('selected',b.dataset.onboardingJourney===state.journey));const next=q('#onboardingNext');if(next&&step===1)next.textContent=state.journey==='not-sure'?'Continue without choosing':state.journey&&state.journey!=='recovery'?`Start ${JOURNEY_LABELS[state.journey]||'this journey'}`:'Continue'}
...
async function preload(){
 ...
 state.path=u.data?.recovery_approach||o.data?.recovery_path||'exploring';state.belief=u.data?.belief_preference||o.data?.belief_preference||'exploring';state.journey=['recovery','caregiving','reentry','housing-stability','independent-living','not-sure'].includes(o.data?.primary_intent)?o.data.primary_intent:null;
...
async function saveSetup(){
 ...
 const {data:started,error:startError}=await sb.rpc('start_selected_support_path',{p_topic_key:'recovery'});if(startError||started?.status!=='started')return toast(startError?.message||'We could not start Recovery. Please try again.',true);
 const rs=await Promise.all([
  ...
  sb.from('onboarding_state').upsert({user_id:currentUser.id,completed:true,completed_at:new Date().toISOString(),primary_intent:'recovery',recovery_path:state.path,belief_preference:state.belief},{onConflict:'user_id'})
 ]);
...
async function startSelectedJourney(){
 const name=q('#onboardingName').value.trim();if(!name)return toast('Add the name you want Lellee to use.',true);if(!state.journey)return toast('Choose what you would like support with.',true);
 const selected=state.journey;const rs=await Promise.all([sb.from('profiles').update({display_name:name}).eq('id',currentUser.id),sb.from('onboarding_state').upsert({user_id:currentUser.id,completed:true,completed_at:new Date().toISOString(),primary_intent:selected},{onConflict:'user_id'})]);const err=rs.find(x=>x.error)?.error;if(err)return toast(err.message,true);
 if(selected==='not-sure'){q('#finalOnboardingOverlay')?.classList.add('hidden');showPage('program-switcher');toast('You can choose a journey whenever you are ready.');return}
 const {data:started,error}=await sb.rpc('start_selected_support_path',{p_topic_key:selected});if(error||started?.status!=='started')return toast(error?.message||'We could not start that journey. Please try again.',true);
 data.name=name;if(typeof save==='function')save();q('#finalOnboardingOverlay')?.classList.add('hidden');try{sessionStorage.setItem('lellee:active-consumer-journey:v1',selected)}catch(_){}location.hash='wave1-beta-workspace';location.reload();
}
function next(){if(step===1){if(!q('#onboardingName').value.trim())return toast('Add the name you want Lellee to use.',true);if(!state.journey)return toast('Choose what you would like support with.',true);if(state.journey!=='recovery')return startSelectedJourney();showStep(2);return}if(step===2&&!q('#onboardingRecoveryDate').value)return toast('Choose a recovery date or use today.',true);if(step<5)showStep(step+1);else saveSetup()}
